...
def grid_search_best_pair():
    ...
    QUALITIES = ["dominant7", "major7", "minor7"]
    ...
    for c1 in ALL_CHORDS:
        for c2 in ALL_CHORDS:
            if c2[0] == c1[0]:
                continue
            for c3 in ALL_CHORDS:
                if c3[0] == c1[0] or c3[0] == c2[0]:
                 continue
                for c4 in ALL_CHORDS:
                    if c4[0] == c1[0] or c4[0] == c2[0] or c4[0] == c3[0]:
                        continue
                    adjusted_scores = compute_posterior([c1, c2, c3, c4], PRIORS)
                    val = adjusted_scores["C major"] * adjusted_scores["A minor"]
                    ...
                    if val > best_value:
                        best_value = val
                        best_pair = (c1, c2, c3, c4)
                        print(best_pair, best_value)
    print('end')
