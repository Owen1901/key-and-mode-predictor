...
# Helper: extract all pitch classes used by a chord
def chord_pitches(ch):
    root, quality, bass = ch
    if bass is None:
        return {root}
    else:
        return {root, bass}
...
def grid_search_best_combo():
    # Build ALL_CHORDS
    QUALITIES = ["major", "minor", "dominant7", "major7"]
    ALL_CHORDS = [
        (root, quality, bass)
        for root in PITCHES
        for quality in QUALITIES
        for bass in [None] + PITCHES
    ]
    ...
    # Iterate over combinations of 3 chords
    for combo in combinations(range(len(ALL_CHORDS)), 3):

        # Collect all pitch classes used by these chords
        pset = set()
        total_pitches = 0

        for idx in combo:
            pitches = chord_pitches(ALL_CHORDS[idx])
            pset |= pitches
            total_pitches += len(pitches)

        # If any pitch repeats, the union size will be smaller than total pitches
        if len(pset) < total_pitches:
            continue
...
if __name__ == "__main__":
    best_chords, best_val = grid_search_best_combo()
    print("Best chords:", best_chords)
    print("Best value:", best_val)