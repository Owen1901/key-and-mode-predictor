...
QUALITIES = ["major", "minor", "6", "minor6", "dominant7", "major7", "minor7", "dominant9",
             "major9", "minor9", "add9", "madd9", "11", "minor11", "add11", "sus2", "major7sus2", "dominant7sus2",
             "sus4", "major7sus4", "dominant7sus4"]
...
def main():
    PRIORS = {}
    for p in PITCHES:
        PRIORS[p + " lydian"] = 0.00711026406934 / 12
        PRIORS[p + " major"] = 0.529879601972 / 12
        PRIORS[p + " mixolydian"] = 0.0611702615038 / 12
        PRIORS[p + " dorian"] = 0.0369807033297 / 12
        PRIORS[p + " minor"] = 0.364859169125 / 12
    ...
    while True:
        ...
        # Step 1: compute likelihoods for all keys for THIS chord
        likelihoods = {}

        for key in scores:
            tonic, mode = key.split()

            # Compute intervals for THIS key
            if bass == None:
            ...
            # Compute degree name for THIS key
            deg = degree_name(iv, upper_interval, bass_interval, quality, mode)

            # Step 1: Lookup likelihood for THIS key
            if mode == "lydian":
        ...
        unnormalized = {k: likelihoods[k] * PRIORS[k] for k in scores}

        # Step 3: normalize posteriors
        Z = sum(unnormalized.values())
        posteriors = {k: unnormalized[k] / Z for k in scores}

        # Step 4: update running scores (Bayesian filtering)
        for k in scores:
            scores[k] *= posteriors[k]

        # Step 5: normalize running scores
        Z2 = sum(scores.values())
        for k in scores:
            scores[k] /= Z2
