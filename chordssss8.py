...
def main():
    PRIORS = {}
    for p in PITCHES:
        PRIORS[p + " lydian"] = 0.00819592019101 / 12
        PRIORS[p + " major"] = 0.510436883037 / 12
        PRIORS[p + " mixolydian"] = 0.0575110651904 / 12
        PRIORS[p + " dorian"] = 0.0447075578392 / 12
        PRIORS[p + " minor"] = 0.379148573742 / 12

    # Initialize scores
    scores = {}
    for p in PITCHES:
        scores[p + " lydian"] = 1 / 60
        scores[p + " major"] = 1 / 60
        scores[p + " mixolydian"] = 1 / 60
        scores[p + " dorian"] = 1 / 60
        scores[p + " minor"] = 1 / 60
    ...
    while True:
        ...
        # Recompute scores from scratch using full history
        scores = {}
        ...
        for key in PRIORS:
            ...
            # Multiply likelihoods for every chord in history
        ...
        for key, val in scores.items():
            ...
            #adjusted_scores[key] = val / np.sqrt(PRIORS[tonic + " " + mode])

        # Normalize
        Z = sum(adjusted_scores.values())
        for key in adjusted_scores:
            adjusted_scores[key] /= Z
