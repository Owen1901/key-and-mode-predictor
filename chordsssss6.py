...
# -------------------------
QUALITIES = [
    "minor6","6","major7","minor7","dominant7","major9","minor9",
    "minoradd9","add9","dominant9","minor11","minoradd11","add11","11",
    "major7sus2","minor7sus2","dominant7sus2","sus2",
...
LYDIAN_PROBS = {
    ...
    "iv^o6_4": 0.005, "iv^o": 0.006, "vii^o": 0.002,
...
LOCRIAN_PROBS = {
    ...
    "i^o": 0.28, "#ii^o": 0.02, "i^o6": 0.02, "i^o6_4": 0.02, "#iv^o": 0.008, "vii^o": 0.009, "vii^o/i": 0.02,
...
def parse_full_chord(ch):
    """
    Parse a chord string possibly containing a slash bass.
    Returns (root, quality, bass) where bass is None or a pitch name.
    """
    ch = ch.strip()
    if "/" in ch:
...
def weights():
    weights = {}
    factor = (143/133)*(53067/49921)*(698894/658139)
    for p in PITCHES:
        ...
        # minor, phrygian and locrian, and so that a major triad built on the root is scored
        # equally in major compared to a minor triad built on the root in minor (ex: C gets
        # the same score in C major as Am gets in A minor)
...
# -------------------------
# ALL_CHORDS builder (supports slash chords)
# -------------------------
...
# -------------------------
# Likelihood matrix builder
# -------------------------
def build_likelihood_matrix(all_chords, priors_keys):
    """
    Returns:
      keys: list of priors_keys (order preserved)
      LIK: numpy array shape (K, C) where LIK[k, c] = P(chord_c | key_k)
    """
...
# -------------------------
# Posterior computation (log domain)
# -------------------------
def compute_posterior_for_combo(combo_indices, logPRIORS, logLIK):
    """
    combo_indices: iterable of chord indices
    logPRIORS: (K,) numpy array
    logLIK: (K, C) numpy array
    returns: normalized posterior (K,) numpy array
    """
...
# -------------------------
# Distinct-pitch constraint (supports slash chords)
# -------------------------
...
# -------------------------
# Grid search (vectorized core uses precomputed logLIK)
# -------------------------
...
# -------------------------
# Plotting utilities (preserve your plotting style)
# -------------------------
...
def plot_results(adjusted_scores):
    ...
    Produces the four plots you had: per-mode probabilities, parent-major histogram,
...
# -------------------------
# Interactive scoring (preserves original interactive loop)
# -------------------------
...
# -------------------------
# Main entry: run grid search and print best combination
# -------------------------
def best_chords_main():
    ...
    # Run grid search for 4-chord progressions (distinct pitch constraint enforced)
    best_chords, best_val = grid_search_best_n(all_chords, priors, LIK, n=4, target_keys=("C major", "D dorian"))
    print("\n=== GRID SEARCH RESULT ===")
    print("Best 2-chord progression (root, quality, bass):")
    print(best_chords)
    print("Best objective value (P(C major)*P(A minor)):", best_val)

if __name__ == "__main__":
    #interactive_main() to input a chord progression and return mode and key probabilities.
    #best_chords_main() to find best chords to maximize indifference to the chord progression
    #being in two specified keys.
    #interactive_main()
    interactive_main()